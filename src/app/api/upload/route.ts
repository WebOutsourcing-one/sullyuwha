 ...
 * 업로드 허용 최대 크기 (30MB).
 *
 * 상품 사진은 스튜디오에서 리터칭해 받는 파일이라 20~30MB가 흔하다.
 * 10MB로 두면 그런 원본이 그대로 막힌다.
 *
 * 이 값은 **1차 방어선일 뿐**이다. 서버 부담을 실제로 정하는 것은 바이트가 아니라
 * 디코딩했을 때의 픽셀 수라서, 진짜 상한은 `MAX_INPUT_PIXELS`가 잡는다.
 * (JPEG은 libvips가 축소하며 디코딩하므로 큰 파일이라도 전체를 펼치지 않는다)
 */
 ...
 *
 * 화면이 실제로 요구하는 최대치에서 나온 숫자다 — 이미지가 가장 크게 걸리는 곳은
 * 상세 갤러리의 `46vw`이고, 2560px 모니터에서 1178 CSS px, 2배 화면에서 2356px다.
 * 2400은 그 위를 살짝 덮는다. 확대(라이트박스) 기능은 없다.
 *
 * ⚠️ 축소는 되돌릴 수 없다. 자수 디테일을 확대해 보여주는 기능을 넣게 되면
 *    그때는 이 값을 먼저 올려야 하고, 이미 올라간 사진은 다시 받아야 한다.
 *    스튜디오 원본은 별도 보관처에 남겨 두는 편이 안전하다.
 */
...
/**
 * 디코딩을 허용할 최대 픽셀 수 (40MP).
 *
 * **서버 부담을 실제로 정하는 값이다.** 메모리는 파일 크기가 아니라 펼쳐진 픽셀에
 * 비례한다 — 40MP면 RGBA로 약 160MB이고, 변환을 한 장씩 돌리므로 이 값이 곧
 * 최대 순간 사용량이다. 흔한 스튜디오 원본(6000×4000 = 24MP)은 넉넉히 통과한다.
 *
 * sharp 기본값 268MP는 이 인스턴스에서 감당할 수 없다. 압축률이 아주 높은 파일
 * 하나가 바이트 상한을 통과하고도 메모리를 통째로 가져갈 수 있어, 바이트만으로는
 * 부족하다.
 *
 * ⚠️ 총 914Mi 인스턴스에는 여유가 많지 않다. 이 값이나 MAX_UPLOAD_BYTES를 더
 *    올리려면 스왑을 먼저 붙이는 편이 안전하다.
 */
