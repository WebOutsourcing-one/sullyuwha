...
export const INTRO_SEEN_KEY = "sullyuwha:intro-seen";
 ...
 *
 * `beforeInteractive` Script는 Next.js 규칙상 루트 레이아웃에서만 지원된다
 * (다른 컴포넌트에 두면 "script tag" 하이드레이션 경고가 난다) — 그래서 이 상수만
 * 여기서 내보내고 실제 `<Script>` 렌더링은 `app/layout.tsx`가 맡는다.
 */
export const INTRO_GATE_SCRIPT = `try{if(sessionStorage.getItem(${JSON.stringify(INTRO_SEEN_KEY)}))document.documentElement.setAttribute('data-intro-seen','')}catch(e){}`;
